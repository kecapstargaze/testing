...
const KomunitasMedia = ({ komunitas, categories }) => {
  const [tab, setTab] = useState("");
  const filterPost = !tab
    ? komunitas
    : komunitas.filter((post) => post.data.categories.includes(tab));
  return (
            ...
            >
              <div className="rounded-xl bg-white px-10 pb-8 pt-11 shadow-lg">
                <div className="integration-card-head flex items-center space-x-4">
                  ...
                  <div>
                    <h2 className="h4">{humanize(item.data.name)}</h2>
